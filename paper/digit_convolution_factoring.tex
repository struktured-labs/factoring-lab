...
\item \textbf{Transfer matrix spectral analysis with closed-form constant.} We prove $\log_2|\Lambda_n \cap \mathcal{B}| = d^2/4 + O(d)$ for base~2, establishing the exact constant $\alpha = 1/4$ via a row-sum identity and binomial parity argument (Theorem~\ref{thm:transfer}).  For general base~$b$, we derive $\alpha(b) = \log_2((b{-}1)^2+1)/4$.

\item \textbf{Information-theoretic carry channel analysis.}  Using the forward--backward algorithm and the Diaconis--Fulman eigenvalue theory~\cite{diaconis09}, we prove the carry chain reveals only $\Theta(d \log d)$ bits of a $\Theta(d^2)$-bit lattice---a vanishing fraction $O(\log d / d)$ (Theorem~\ref{thm:channel}).

\item \textbf{SOS rounding hardness.}  Degree-4 SOS closes the continuous gap to $<0.1\%$, but we prove sequential rounding fails with probability $2^{-\Theta(d^2)}$ (Theorem~\ref{thm:rounding}).  To our knowledge, this is the first explicit certification-vs-search separation demonstrated in a number-theoretic recovery formulation.

\item \textbf{Base-dependent spectral landscape.} We prove that base~2 has zero spurious near-rank-1 lattice points ($\sigma_2/\sigma_1 < 0.1$), while base~10 has up to 79\% spurious near-rank-1 points, explaining the failure of nuclear norm minimization in larger bases (Theorem~\ref{thm:spectral}).
...
\subsection{Transfer Matrix and Carry Channel Analysis}
...
\]
This replaces the heuristic estimate of Theorem~\ref{thm:sparsity}(b) with a \textbf{rigorous, polynomial-time computation}: $O(d \cdot S^2)$ where $S = \max_k t_k^{\max}$.
...
\subsection{Carry Channel Capacity Bound}
...
\subsection{SOS Hierarchy and Rounding Hardness}
...
\subsection{Base-Dependent Spectral Landscape}
...
\begin{theorem}[Base-dependent spectral landscape]\label{thm:spectral}
...
\subsection{Blind Deconvolution and SVD Recovery}
...
\]
which is proven rigorously by Theorem~\ref{thm:transfer}.
\end{enumerate}
\end{theorem}
...
Part (b): The lattice point count satisfies $|\Lambda_n \cap \mathcal{B}| = 2^{d^2/4 + O(d)}$ by Theorem~\ref{thm:transfer}, giving the ratio $\le 2/2^{d^2/4 + O(d)} = 2^{-\Omega(d^2)}$.
\end{proof}
