...
\item \textbf{SOS rounding hardness.}  Degree-4 SOS closes the continuous gap to $<0.1\%$, but we prove sequential rounding fails with probability $2^{-\Theta(d^2)}$ (Theorem~\ref{thm:rounding}).  This is the first certification-vs-search gap for a number-theoretic problem.
...
\textbf{Certification vs.\ search.}  The SOS gap measures \emph{certification}: how well the relaxation certifies that a solution exists.  Rounding measures \emph{search}: extracting the solution from the certificate.  Our result demonstrates a $\Theta(d^2)$-bit gap between certification (nearly perfect) and search (exponentially hard).  This is the first such demonstration for a number-theoretic problem, paralleling results for planted clique~\cite{barak16} and tensor PCA~\cite{hopkins15}.
